...
Get-ChildItem -Path $folder -Filter *.csar | ForEach-Object {
    ...
    $cmd1 = '"' + $carisBatch + '" --run AddComputedBand --compute-band "' + $band1Name + '" --expression ' + $band1Expr + ' --z-axis-convention DOWN --computed-band-type ' + $band1Type + ' "' + $file + '"'
    ...
    $cmd2 = '"' + $carisBatch + '" --run AddComputedBand --compute-band "' + $band2Name + '" --expression ' + $band2Expr + ' --z-axis-convention DOWN --computed-band-type ' + $band2Type + ' "' + $file + '"'
